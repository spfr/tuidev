...
- **A hand-edited `~/.claude/settings.json` is never overwritten.** An unmodified shipped copy is replaced by `./scripts/update.sh --configs`; one you edited gets the shipped changes merged in, the `sandbox` block included, and keeps your own values (see [updating.md](updating.md#shipped-configs-you-may-have-edited)). If the merge can't run (the file isn't valid JSON, or `jq` is missing), add the `sandbox` block yourself. Until then, plain `claude` runs unsandboxed.

**Machine-specific exceptions go in your own `~/.claude/settings.json`**: an extra `sandbox.excludedCommands` entry (say `"git *"`), a `sandbox.filesystem.allowRead` or `allowWrite` path such as `~/work/shared-cache`, a domain under `sandbox.network.allowedDomains`. Updates merge the shipped changes into that file and keep your entries, so they don't need a fork of `configs/claude/settings.json`. The one exception: an entry a release removes from the shipped file is dropped, even if you had it before (see [updating.md](updating.md#shipped-configs-you-may-have-edited)).
...
**On Linux, the `.env` rules are literal paths.** Linux installs get `configs/claude/settings.linux.json`, which is the same file except for those rules. It denies `.env`, `.env.local`, `.env.development`, `.env.development.local`, `.env.test`, `.env.test.local`, `.env.production`, `.env.production.local` and `.env.staging`. Those paths are relative to the directory you start `claude` in, and the rules cover `Read`, `Edit` and Bash (`cat .env` fails). The `Read` and `Edit` denials always apply. Bash is denied only files that exist when the session starts, so treat a `.env` created mid-session as possibly readable from Bash. Claude Code's bubblewrap sandbox can deny whole directories (`~/.ssh/**`) and literal files, but not a filename pattern. When a Read or Edit rule has a pattern such as `**/.env`, Claude Code walks the whole project, including `node_modules` and `.git`, looking for matches. It does this again and again while the model works. On a large repo that keeps one core busy and makes typing lag by seconds. On one monorepo, the walk opened about 276k directories in 45 seconds, and the prompt never ran. The trade-off is that a `.env` in a subdirectory, or with another name, isn't denied on Linux. Keep secrets out of the tree the agent works in, or add literal rules for the paths you have (`Read(./apps/api/.env)`). Don't add `**` patterns back. If Claude Code warns at startup that "glob patterns in sandbox permission rules are not fully supported", a pattern is still in your settings. `--pack ai-clis` warns about the `.env` pattern rules when your `~/.claude/settings.json` is your own copy. It doesn't check a project's own `.claude/settings.json`, which triggers the same walk. macOS keeps the patterns: Seatbelt matches them natively, without a walk. Both variants are keyed as the same shipped file, so if you share one `~/.claude/settings.json` between a Mac and a Linux box through dotfiles, each `update.sh --configs` swaps an unmodified copy for that platform's and leaves a backup. Edit it once to make it yours: from then on each machine merges its variant's changes in instead, and the `.env` rules of both end up in the file, so on Linux `--pack ai-clis` keeps warning until you remove the `**` ones.
