...
- **No verification mandates or spawn caps in prose.** Anthropic's Claude 5 notes say these models check their own work and delegate readily, and that telling them to verify through a subagent causes over-verification. So the `reviewer` is offered, not required, and how wide a fan-out goes is Claude Code's setting (the workflow size in `/config`), not policy text.
