...
6. **User-agnostic paths.** All configs use `$HOME`; no hardcoded `/Users/NAME` strings (CI enforces this). Personal LAN hosts belong in `~/.ssh/config.local` / `~/.zshrc.local`. Machine-specific notes, scripts and agent instructions live in the gitignored `.local/` folder (and `CLAUDE.local.md` for Claude Code); this repo ships best practices only, never personal hosts, services or hardware details.
