...
LAYOUT_DEFAULT_NAME="ai-triple"
# shellcheck disable=SC2034
LAYOUT_USAGE="[SESSION_NAME]    # nvim(55%) + 3 stacked agent panes (45%)"
...
run_cmd tmux split-window -t "${SESSION_NAME}:0.1" -v -p 67 -c "$PWD"
run_cmd tmux split-window -t "${SESSION_NAME}:0.2" -v -p 50 -c "$PWD"
